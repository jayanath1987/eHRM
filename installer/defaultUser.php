...
function submitDefUserInfo() {
	...
	if(frm.OHRMAdminUserName.value.length < 5) {
		alert('eHRM Admin User-name should be at least 5 char. long!');
		frm.OHRMAdminUserName.focus();
		return;
	}

	if(frm.OHRMAdminPassword.value == '') {
		alert('eHRM Admin Password left Empty!');
		frm.OHRMAdminPassword.focus();
		return;
	}

	if(frm.OHRMAdminPassword.value != frm.OHRMAdminPasswordConfirm.value) {
		alert('eHRM Admin Password and Confirm eHRM Admin Password don\'t match!');
		frm.OHRMAdminPassword.focus();
		return;
	}
...
<div id="content">
	<h2>Step 4: Admin User Creation</h2>
        ...
        <p>After eHRM is configured you will need an Administrator Account to Login into eHRM. <br />
