...
function fillData($phase=1, $source='/dbscript/dbscript-') {
	...
	for($c=0;(count($dbScriptStatements)-1)>$c;$c++)
		...
		if($phase==1){
               $triggerQueryFile = ROOT_PATH . '/dbscript/triggers.sql';
	$fp    = fopen($triggerQueryFile, 'r');

	//error_log (date("r")." Fill Data Phase $phase - Opened DB Script\n",3, "installer/log.txt");

	$triggerQuery = fread($fp, filesize($triggerQueryFile));
	fclose($fp);

$mysqli  = mysqli_connect( $_SESSION['dbInfo']['dbHostName'],$_SESSION['dbInfo']['dbUserName'] , $_SESSION['dbInfo']['dbPassword'], $_SESSION["dbInfo"]["dbName"],$_SESSION['dbInfo']['dbHostPort']);
    ...
    die('Connect Error (' . mysqli_connect_errno() . ') '
            . mysqli_connect_error());
             ...
             $query = mysqli_multi_query( $mysqli, $triggerQuery ) or die( mysqli_error( $mysqli ) );
...
function createUser() {
	...
	if(!@mysql_select_db($_SESSION['dbInfo']['dbName'])) {
		$_SESSION['error'] = 'Unable to access eHRM Database!';
		return;
	}
...
$query="INSERT INTO `hs_hr_users` (`id`, `user_name`, `user_password`, `emp_number`, `is_admin`, `date_entered`, `date_modified`, `modified_user_id`, `created_by`, `status`, `userg_id`, `sm_capability_id`,`user_prefered_language`) VALUES('USR001', '" .$_SESSION['defUser']['AdminUserName']. "', '" .md5($_SESSION['defUser']['AdminPassword']). "', NULL, 'Yes', NULL, NULL, NULL, NULL, 'Enabled', 'USG001', NULL, 'en')";

	if(!mysql_query($query)) {
		$_SESSION['error'] = 'Unable to Create eHRM Admin User Account';
		return;
	}
...
function writeLog() {
   ...
   if (isset($_SESSION['INSTALLING'])) {
	switch ($_SESSION['INSTALLING']) {
		...
		case 4	:	error_log (date("r")." Create eHRM user - Starting\n",3, "installer/log.txt");
					createUser();
					error_log (date("r")." Create eHRM user - Done\n",3, "installer/log.txt");
					if (!isset($error) || !isset($_SESSION['error'])) {
						$_SESSION['INSTALLING'] = 5;
						error_log (date("r")." Create eHRM user - No Errors\n",3, "installer/log.txt");
					} else {
						error_log (date("r")." Create eHRM user - Errors\n",3, "installer/log.txt");
						error_log (date("r")." ".(isset($error)? $error: $_SESSION['error'])."\n",3, "installer/log.txt");
					}
					break;
