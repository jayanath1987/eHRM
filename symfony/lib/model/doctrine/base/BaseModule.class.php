<?php

/**
 * This class has been auto-generated by the Doctrine ORM Framework
 */
abstract class BaseModule extends sfDoctrineRecord
{
    public function setTableDefinition()
    {
        $this->setTableName('hs_hr_module');
        $this->hasColumn('mod_id', 'string', 36, array(
             'type' => 'string',
             'primary' => true,
             'length' => '36',
             ));
        $this->hasColumn('name', 'string', 45, array(
             'type' => 'string',
             'length' => '45',
             ));
        $this->hasColumn('module_name_si', 'string', 100, array(
             'type' => 'string',
             'length' => '100',
             ));
        $this->hasColumn('module_name_ta', 'string', 100, array(
             'type' => 'string',
             'length' => '100',
             ));
        $this->hasColumn('owner', 'string', 45, array(
             'type' => 'string',
             'length' => '45',
             ));
        $this->hasColumn('owner_email', 'string', 100, array(
             'type' => 'string',
             'length' => '100',
             ));
        $this->hasColumn('version', 'string', 36, array(
             'type' => 'string',
             'length' => '36',
             ));
        $this->hasColumn('description', 'string', 2147483647, array(
             'type' => 'string',
             'length' => '2147483647',
             ));
    }

    public function setUp()
    {
        $this->hasMany('ModuleRights', array(
             'local' => 'mod_id',
             'foreign' => 'mod_id'));

        $this->hasOne('WfModule', array(
             'local' => 'mod_id',
             'foreign' => 'wfmod_id'));

        $this->hasMany('menuitem', array(
             'local' => 'mod_id',
             'foreign' => 'mod_id'));

        $this->hasMany('FormLockDetails', array(
             'local' => 'mod_id',
             'foreign' => 'mod_id'));

        $this->hasMany('ReportDetails', array(
             'local' => 'mod_id',
             'foreign' => 'mod_id'));
    }
}