...
class EmployeeTableTest extends PHPUnit_Framework_TestCase {
    ...
    /**
     * TestCase for getEmployeeCount() function
     */
    public function testGetEmployeeCount() {
        ...
        // Count all employees
        $count = Doctrine::getTable('Employee')->getEmployeeCount();
        $expected = count($employees);
        $this->assertEquals($expected, $count, 'Count is wrong');

        // count only employees matching search
        ...
        foreach ($filters as $filter) {
                $count = Doctrine::getTable('Employee')->getEmployeeCount($filter);
                $expected = $this->_filterList($employees, $filter);
                $this->assertEquals(count($expected), $count, 'Count does not match');
        }

        // verify count does not count terminated employees
        $terminated = array($employees[2], $employees[4]);

        $ids[] = $terminated[0]['empNumber'];
        $ids[] = $terminated[1]['empNumber'];
        ...
        $count = Doctrine::getTable('Employee')->getEmployeeCount();
        $expected = count($this->employees) - 2;
        $this->assertEquals($expected, $count, 'Count does not match');
    }
    ...
    public function testListHidesTerminatedEmployees() {
        ...
        $terminated[0]['employeeStatus'] = 'Terminated';
        $terminated[1]['employeeStatus'] = 'Terminated';
...
//
    ...
    private function _compareLists($results, $expected, $msg = '') {
        ...
        for ($i = 0; $i < $size; $i++ ) {
            ...
            $workStation = $actual->getWorkStation();
            $this->assertEquals($exp['subDivision'], $workStation->getTitle(), $msg);
        }
    }
