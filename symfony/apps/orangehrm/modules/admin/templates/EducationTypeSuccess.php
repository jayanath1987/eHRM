...
<div class="outerbox">
    <div class="maincontent">
        ...
        <div class="mainHeading"><h2><?php echo __("Education Types") ?></h2></div>
        <?php echo message(); ?>
        <form name="frmSearchBox" id="frmSearchBox" method="post" action="">
            ...
            <div class="searchbox">
                ...
                <select name="cmbSearchMode" id="cmbSearchMode">
                    ...
                    <option value="edu_type_name" selected><?php echo __("Education Type") ?></option>
                </select>
        ...
        <br class="clear" />
        <form name="standardView" id="standardView" method="post" action="<?php echo url_for('admin/DeleteEducationType') ?>">
...
<?php echo $sorter->sortLink("edu_type_name", __('Education Type Name'), '@EducationType', ESC_RAW); ?>
                        </td>
                    </tr>
                </thead>
                ...
                <tbody>
                    <?php
                    $row = 0;
                    foreach ($ListEducationType as $EducationType) {
                        $cssClass = ($row % 2) ? 'even' : 'odd';
                        $row = $row + 1;

                        //Define data columns according culture
                        $ETNameCol = ($userCulture == "en") ? "edu_type_name" : "edu_type_name_" . $userCulture;
                        $ETName = $EducationType->$ETNameCol == "" ? $EducationType->edu_type_name : $EducationType->$ETNameCol;

                        $id = $EducationType->edu_type_id;
                    ?>

                        <tr class="<?php echo $cssClass ?>">
                            <td >
                                <input type='checkbox' class='checkbox innercheckbox' name='chkID[]' id="chkID" value='<?php echo $EducationType->edu_type_id ?>' />
                            </td>

                            <td class="">
                                <a href="<?php echo url_for('admin/SaveEducationType?ETId=' . $encrypt->encrypt($EducationType->edu_type_id)) ?>"><?php echo $ETName ?></a>
                            </td>
                        </tr>
<?php } ?>
                </tbody>
...
<script type="text/javascript">
    ...
    $(document).ready(function() {
        ...
        $("#btnAdd").click(function() {
            location.href = "<?php echo url_for(public_path('../../symfony/web/index.php/admin/SaveEducationType')) ?>";
        ...
        $("#btnReset").click(function() {
            location.href = "<?php echo url_for(public_path('../../symfony/web/index.php/admin/EducationType')) ?>";
        });
